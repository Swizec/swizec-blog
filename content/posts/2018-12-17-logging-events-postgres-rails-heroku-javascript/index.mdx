...
hero: ./img/screenshot-1545070245704.png
---

https&#x3A;//twitter.com/Swizec/status/1049786180483477504 When I was a wee lad of 21 or 23 I built my first startup. Designed for high scalability, we used Google App Engine for infinite horizontal scaling, Big Table to hold our oodles of data, many queues and sharding databases and tiny function objects that run independently on an infinite number of machines to deal with our massive scale.  
...
[![](./img/screenshot-1545070245704.png)  
...
[![](./img/screenshot-1545070247810.png)  
...
[![](./img/screenshot-1545070246062.png)  
...
[![](./img/screenshot-1545070245435.png)  
...
[![](./img/screenshot-1545070249232.png)](https://carbon.now.sh/?bg=rgba(255,255,255,1)&t=seti&l=ruby&ds=true&wc=true&wa=true&pv=48px&ph=32px&ln=false&code=%20(0..3).each%20do%20%7Cid%7C%0A%20%20Rails.application.config.log_entries%5B%3Abatch_seconds%5D.each%20do%20%7Cn%7C%0A%20%20%20%20%22LogEntriesBatchWorker%23%7Bid%7D%22.constantize.perform_in(n.seconds)%0A%20%20end%0Aend%0A) Each worker has an ID, that helps it pick events based on a modulus.  
[![](./img/screenshot-1545070329469.png)](https://carbon.now.sh/?bg=rgba(255,255,255,1)&t=seti&l=ruby&ds=true&wc=true&wa=true&pv=48px&ph=32px&ln=false&code=def%20self.process_batch(index%2C%20batch_size%3D500%2C%20type%3A%20%3Adefault)%0A%20%20ids%20%3D%20LogEntryStaging.not_processed%0A%20%20%20%20%20%20.where(staging_type%3A%20type)%0A%20%20%20%20%20%20.limit(batch_size*4)%0A%20%20%20%20%20%20.select(%3Aid)%0A%20%20%20%20%20%20.select%20%7B%20%7Centry%7C%20entry.id%254%20%3D%3D%20index%20%7D%0A%0ALogEntryStaging.where(id%3A%20ids).each%20do%20%7Crow%7C%0A) Take unprocessed staged logs, limit to 4x of batch size, gets their ids, keep every 4th id. Select those staged logs and process them.  
[![](./img/screenshot-1545070345134.png)  
](<https://d.pr/i/3kEvbf.jpg >)  
You now have 4-way parallelism and queues are draining faster than they're filling 👌  
[![](./img/screenshot-1545070345337.png)  
