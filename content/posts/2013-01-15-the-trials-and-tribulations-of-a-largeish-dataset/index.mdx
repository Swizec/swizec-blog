...
[![Commit time histogram](./img/blog-wp-content-uploads-2013-01-commit-histogram.png "Commit time histogram")](http://nightowls.swizec.com/histogram/hours)



The plan was to scrape a massive [dataset](http://en.wikipedia.org/wiki/Data_set "Data set") off [Github](http://github.com "GitHub") and do some analysis of programmers' working habits, at least the ones using Github that is. As such, the script was fairly simple - run a search query via the API, get page 1, 2, 3 ...
...
\[caption id="attachment_5855" align="alignnone" width="650"][![This is a punchcard](http://swizec.com/blog/wp-content/uploads/2013/01/swizec-punchcard.png "This is a punchcard")](./img/blog-wp-content-uploads-2013-01-swizec-punchcard.png) This is a punchcard\[/caption]
...
Although not a very big dataset by most means, at half a million, it was enough to thwart my original approach of using [Mongoose](http://mongoosejs.com/) to handle my mongodb connection.
...
Alas. After running a script for a few minutes - oh yes, did I mention it takes about 3 minutes just to read through the entire list of repositories? - it turns out the [Redis](http://redis.io/ "Redis (data store)") set was so massive my attempts at reading it were to no avail. Ran out of memory before even doing anything serious.
