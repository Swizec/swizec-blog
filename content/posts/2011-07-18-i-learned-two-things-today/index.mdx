...
_tl;dr --> no matter what it seems like, the first price a client suggests is never fair._
...
To make it more fun, the admin interface is made so every form can only handle a single [database table](http://en.wikipedia.org/wiki/Table_%28database%29 "Table (database)").



Naturally my extension requires a [data model](http://en.wikipedia.org/wiki/Data_model "Data model") encompassing many interconnected tables.
...
Not to mention that user passwords are stored in [plaintext](http://en.wikipedia.org/wiki/Plaintext "Plaintext") in the database and the admin's password is stored in a config file in plaintext ... oh yeah. Gotta love that!
