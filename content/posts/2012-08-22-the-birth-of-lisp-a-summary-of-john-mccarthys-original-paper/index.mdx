...
---
> A programming system called [LISP](http://en.wikipedia.org/wiki/Lisp_%28programming_language%29 "Lisp (programming language)") (for LISt Processor) has been developed for the IBM 704 computer by the [Artificial Intelligence](http://en.wikipedia.org/wiki/Artificial_intelligence "Artificial intelligence") group at M.I.T. The system was designed to facilitate experiments with a proposed system called the Advice Taker, whereby a machine /../ could exhibit “common sense” in carrying out its instructions.
>
>
>
> /../
...
>
>
> LISP eventually came to be based on a scheme for representing the [partial recursive functions](http://en.wikipedia.org/wiki/Computable_function "Computable function") of a certain class of symbolic expressions. It now seems expedient to expound the system by starting with the class of expressions called S-expressions and the functions called S-functions.
...
By introducing **Recursive S-functions** we finally get a strong enough class of functions that we can construct all computable functions. McCarthy now defines a bunch of useful functions_-_let's look at the example of _ff_, which returns the first element of an S-expression.
...
The paramount example is _maplist_(called "map" these days), which applies a function to every member of a list to construct a different list - for instance increasing all values by +1.
...
\[caption id="attachment_5133" align="alignnone" width="465" caption="List structure"][![List structure](http://swizec.com/blog/wp-content/uploads/2012/08/list-structure.png "List structure")](./img/blog-wp-content-uploads-2012-08-list-structure.png)\[/caption]
...
\[caption id="attachment_5134" align="alignnone" width="165" caption="((A · B) · (C · (E · F)))"][![((A · B) · (C · (E · F)))](http://swizec.com/blog/wp-content/uploads/2012/08/example-list-structure.png "((A · B) · (C · (E · F)))")](./img/blog-wp-content-uploads-2012-08-example-list-structure.png)\[/caption]
...
\[caption id="attachment_5136" align="alignnone" width="370" caption="Association list for "DIFFERENTIATE""][![Association list for "DIFFERENTIATE"](http://swizec.com/blog/wp-content/uploads/2012/08/association-list.png 'Association list for "DIFFERENTIATE"')](./img/blog-wp-content-uploads-2012-08-association-list.png)\[/caption]
...
It's easy to imagine how the **elementary S-functions work in the Computer**_._They amount to nothing more than returning the correct part of the word. So _car_ returns the address value, _cdr_ the decrement value, _cons_ takes from free storage and fills the values ... and so on.
...
Another interesting observation is that talking about **representation of S-functions by Programs** McCarthy ever so casually introduces lazy evaluation by saying that compilation is simple and _"conditional expressions must be so compiled that only the_p's_and_e's_that are required are computed"_
...
Now if only there weren't so many damn parentheses ... [I tried to learn LISP (clojure really)](http://swizec.com/blog/tag/clojure), I really did. But the syntax was just too jarring.
